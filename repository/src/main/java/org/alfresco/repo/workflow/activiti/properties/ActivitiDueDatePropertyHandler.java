...
public class ActivitiDueDatePropertyHandler extends ActivitiTaskPropertyHandler
{
    ...
    @Override protected Object handleTaskProperty(Task task, TypeDefinition type, QName key, Serializable value)
    {
        ...
        recordTaskDueDateChange(task.getId(), (Date) value);
        return DO_NOT_ADD;
    }
    ...
    private void recordTaskDueDateChange(String id, Date date)
    {
        CommandContext commandContext = Context.getCommandContext();
        if (commandContext != null)
        {
            commandContext.getHistoryManager().recordTaskDueDateChange(id, date);
        }
    }
