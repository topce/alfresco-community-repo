...
public class ActivitiEngineInitializer extends AbstractLifecycleBean
{
    ...
    protected void onBootstrap(ApplicationEvent event) {
    	...
    	{
    		((ProcessEngineImpl)processEngine).getProcessEngineConfiguration().getAsyncExecutor().start();
    	}
    }
	...
	protected void onShutdown(ApplicationEvent event) {
		if(workflowAdminService.isEngineEnabled(ActivitiConstants.ENGINE_ID) && 
				((ProcessEngineImpl)processEngine).getProcessEngineConfiguration().getAsyncExecutor().isActive())
		{
			((ProcessEngineImpl)processEngine).getProcessEngineConfiguration().getAsyncExecutor().shutdown();
		}
	}
}
