...
public class AlfrescoProcessBpmnParseHandler extends AbstractBpmnParseHandler<Process>
{
    ...
    protected void executeParse(BpmnParse bpmnParse, Process process)
    {
        ...
        if (processCreateListener != null)
        {
            List<ActivitiListener> executionListeners = currentProcess.getExecutionListeners();
            executionListeners.add(createStartActivitiListener());
            currentProcess.setExecutionListeners(executionListeners);
        }
        ...
        if (multiTenancyEnabled && tenantService.isEnabled())
        {
            ...
            String processId = tenantService.getName(currentProcess.getId());
            currentProcess.setId(processId);
        }
    }
    ...
    private ActivitiListener createStartActivitiListener()
    {
        ActivitiListener activitiListener = new ActivitiListener();
        activitiListener.setEvent(ExecutionListener.EVENTNAME_START);
        activitiListener.setImplementation("${activitiProcessCreateListener}");
